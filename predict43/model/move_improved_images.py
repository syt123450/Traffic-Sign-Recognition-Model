...
def parse_file_path(root_path, to_path, suffix):
    target_file_list = []
    to_file_list = []
    target_class_list = []
    if os.path.exists(root_path):
        for sub_dir in os.listdir(root_path):
            ...
            sub_to_dir_path = os.path.join(to_path, sub_dir)
            if os.path.isdir(sub_dir_path):
                for target_file in os.listdir(sub_dir_path):
                    if target_file.endswith(suffix):
                        ...
                        to_file_list.append(os.path.join(sub_to_dir_path, target_file))
                        target_class_list.append(sub_dir)
    return target_file_list, to_file_list, target_class_list
...
def move_files(from_list, to_list):
    start = time.time()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_dir", help="source images directory")
    parser.add_argument("--to_dir", help="images directory after movement")
    parser.add_argument("--suffix", help="valid images suffix with '.'")
    parser.add_argument("--eps", help="thread hold for minimum acceptable accuracy to be added to the source")
    ...
    for idx in range(len(target_file_list)):
        ...
        if target_class_list[idx] == labels[top_1]:
            ...
            if results[top_1] >= eps:
                from_list.append(target_file_list[idx])
                to_list.append(target_to_file_list[idx])
        print('{} <> {} ~~ {} ============> {} sec'.format(target_class_list[idx], labels[top_1], results[top_1], time.time() -start_time))
        # for i in top_k:
        #     print(labels[i], results[i])
